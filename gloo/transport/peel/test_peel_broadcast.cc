/**
 * End-to-end sanity test for the standalone Peel transport broadcast.
 *
 * Usage:
...
int main(int argc, char** argv) {
  ...
  std::cout << "[PEEL] rank=" << rank << " world_size=" << worldSize
            << " redis=" << redisHost << ":" << redisPort
            << " iface=" << iface << " mcast=" << mcastGroup
            << " base_port=" << basePort << " topology="
            << (topologyFile.empty() ? "(flat)" : topologyFile) << "\n";
  ...
  discoveryConfig.redis_prefix = "peel_test_fixed/peel_ip";
  ...
  if (!peelContext.init()) {
    std::cerr << "[PEEL] ERROR: PeelContext init failed\n";
    return 3;
  }
  ...
  const bool ok = peelContext.broadcast(kRoot, data.data(), bytes);
  const auto end = Clock::now();
  if (!ok) {
    std::cerr << "[PEEL] ERROR: broadcast failed\n";
    return 4;
  }
  ...
  const auto usec =
      std::chrono::duration_cast<std::chrono::microseconds>(end - start)
          .count();
  const double mbps = static_cast<double>(bytes) / (usec / 1e6) /
      (1024.0 * 1024.0);
  std::cout << "[PEEL] broadcast completed in " << usec / 1000.0
            << " ms (" << mbps << " MB/s)\n";
